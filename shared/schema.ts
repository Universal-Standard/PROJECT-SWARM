...
export const knowledgeEntries = pgTable(
  "knowledge_entries",
  {
    ...
    sourceExecutionId: varchar("source_execution_id").references(() => executions.id, {
      onDelete: "set null",
    }),
...
export const workflowVersions = pgTable(
  ...
  (table) => [index("idx_workflow_versions").on(table.workflowId, table.version.desc())]
);
...
export const webhookLogs = pgTable("webhook_logs", {
  ...
  webhookId: varchar("webhook_id")
    .notNull()
    .references(() => workflowWebhooks.id, { onDelete: "cascade" }),
...
export const executionCosts = pgTable(
  "execution_costs",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => [index("idx_execution_costs").on(table.executionId, table.timestamp.desc())]
);
...
export const providerPricing = pgTable(
  ...
  (table) => [index("idx_provider_pricing").on(table.provider, table.model)]
);
...
export const workflowTags = pgTable(
  ...
  (table) => [index("idx_workflow_tags").on(table.workflowId, table.tagId)]
);
...
export const insertWorkflowWebhookSchema = createInsertSchema(workflowWebhooks).omit({
  id: true,
  createdAt: true,
});
...
export const insertWorkflowSchemaSchema = createInsertSchema(workflowSchemas).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export const insertExecutionCostSchema = createInsertSchema(executionCosts).omit({
  id: true,
  timestamp: true,
});
...
export const insertWorkflowTagSchema = createInsertSchema(workflowTags).omit({
  id: true,
  createdAt: true,
});
