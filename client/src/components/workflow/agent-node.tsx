...
export const AgentNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  const StatusIcon: React.ComponentType<{ className?: string }> | null =
    status === "running"
      ? Loader2
      : status === "completed"
        ? CheckCircle2
  ...
  const handleNameSave = async () => {
    if (editedName.trim() && editedName !== data.label) {
      try {
        ...
        setNodes((nodes) =>
          nodes.map((node) =>
            node.id === id ? { ...node, data: { ...node.data, label: editedName.trim() } } : node
          )
        );
  ...
  const handleProviderChange = async (provider: string) => {
    ...
    try {
      ...
      setNodes((nodes) =>
        nodes.map((node) =>
          node.id === id ? { ...node, data: { ...node.data, provider, model: defaultModel } } : node
        )
      );
  ...
  const handleModelChange = async (model: string) => {
    ...
    try {
      ...
      // Update React Flow node state
      setNodes((nodes) =>
        nodes.map((node) => (node.id === id ? { ...node, data: { ...node.data, model } } : node))
      );
  ...
  return (
    <Card
      className={`min-w-[200px] transition-all ${selected ? "ring-2 ring-primary shadow-xl" : ""}`}
    >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              <Select value={selectedProvider} onValueChange={handleProviderChange}>
                <SelectTrigger
                  className="h-6 w-auto text-xs font-mono"
                  data-testid={`select-provider-${data.agentId}`}
                >
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <Select value={selectedModel} onValueChange={handleModelChange}>
                <SelectTrigger
                  ...
                  data-testid={`select-model-${data.agentId}`}
                >
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
