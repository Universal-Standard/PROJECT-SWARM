...
# Additional Task/Assignment Labels
echo ""
echo "Creating task and assignment labels..."
gh label create "task" -d "General development task" -c "0e8a16" -f || echo "  ✓ task already exists"
gh label create "fix" -d "Non-critical fix needed" -c "fbca04" -f || echo "  ✓ fix already exists"
gh label create "planned-upgrade" -d "Planned system upgrade" -c "0052cc" -f || echo "  ✓ planned-upgrade already exists"
gh label create "feature-request" -d "Community feature request" -c "d876e3" -f || echo "  ✓ feature-request already exists"
gh label create "self-assigned" -d "Self-assigned task" -c "0e8a16" -f || echo "  ✓ self-assigned already exists"
gh label create "auto-assign" -d "Let orchestrator assign" -c "fbca04" -f || echo "  ✓ auto-assign already exists"
gh label create "orchestrator-assigned" -d "Assigned by orchestrator" -c "5319e7" -f || echo "  ✓ orchestrator-assigned already exists"
gh label create "awaiting-assignment" -d "Waiting for assignment" -c "ffeb3b" -f || echo "  ✓ awaiting-assignment already exists"
gh label create "needs-manual-assignment" -d "Needs manual assignment" -c "ff9800" -f || echo "  ✓ needs-manual-assignment already exists"
gh label create "needs-team-assignment" -d "Needs team review" -c "d876e3" -f || echo "  ✓ needs-team-assignment already exists"
gh label create "help-wanted" -d "Open for volunteers" -c "008672" -f || echo "  ✓ help-wanted already exists"
gh label create "good-first-issue" -d "Good for newcomers" -c "7057ff" -f || echo "  ✓ good-first-issue already exists"
...
gh label create "needs-triage" -d "Needs initial triage" -c "ffeb3b" -f || echo "  ✓ needs-triage already exists"
gh label create "needs-info" -d "Needs more information" -c "d876e3" -f || echo "  ✓ needs-info already exists"
gh label create "duplicate" -d "Duplicate issue" -c "cfd3d7" -f || echo "  ✓ duplicate already exists"
gh label create "wontfix" -d "Will not fix" -c "ffffff" -f || echo "  ✓ wontfix already exists"
gh label create "invalid" -d "Invalid issue" -c "e4e669" -f || echo "  ✓ invalid already exists"
...
echo "  - Type (bug, enhancement, task, fix, etc.)"
echo "  - Status (in-progress, in-review, blocked, etc.)"
echo "  - Components (workflow-builder, execution-engine, etc.)"
echo "  - Groups (group-a-core through group-g-future)"
echo "  - Assignment (self-assigned, auto-assign, etc.)"
echo "  - PR sizes and types"
echo "  - Triage workflow"
echo ""
echo "  Total: 70+ labels for comprehensive issue management"
echo ""
echo "Next steps:"
